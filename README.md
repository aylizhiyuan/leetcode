...
总结:如果是遇到算法题了，最先应该想到的看看处理的数据结构是什么类型的？大多数的数据结构无非就是数组、字符串、链表、二叉树、二叉排序树这些最常见的

1. 链表类型的直接使用链表解决即可，想清楚要解决问题的思路，把图画出来，问题基本就解决了，重点要掌握链表结构的逆转、排序、分类和合并

2. 数组类型的数据，通常都是求解一系列满足条件的结果，这时候，可以首先考虑使用暴力的穷举法，当然这么做效率会很低，数组的一般解题方式包括贪心、递归、动态规划，这种问题一般穷举法很难解决问题，递归是首要考虑的解决思路，递归无果考虑动态规划。同时，我们要清楚的意识到，递归是一种考虑两种情况的思想，放入不放入，左子树或者是右子树，加1或者是不加1，等等之类的问题通常都会使用递归思想解决

3. 二叉树本身就是用递归问题解决的，所以，一般二叉树的问题基本都是围绕遍历二叉树节点的情况下解决的

4. 二叉排序树的问题通常会使用二分查找法解决，同时跟查找有关的都可以使用二分查找方式

5. 字符串问题首先考虑哈希表，一般都可以顺利得到解决

其中较难的属于是动态规划，要了解使用动态规划的原则是该问题的解决取决于上一步的结果，用上一步或者更多步的结果去解决当前的问题，也叫归纳思想。核心就是找到动态规划的式子，根据式子来解决，防止使用递归这种方式来解决。
